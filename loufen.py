...
#数据库联结
database = torndb.Connection("127.0.0.1", "databaseloufen", user="root", password="SFC2017") 
    ...
    def post(self):
        ...
        if tags=="username":
             ...
             if database:
                 ...
                 d = database.query("SELECT * FROM usernamepw where username=%s",username);
                 if d:
...
class StoreAValue(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if tag=="longlat":
            ...
            if database:
                rec=database.query("SELECT * FROM traceinfo WHERE phonenumber=%s",phonenumber);
                if rec: 
                   database.execute("UPDATE traceinfo SET latlong=%s WHERE phonenumber=%s",latlong,phonenumber);
                   
                else:
                   self.set_header('Content-Type','application/json')
                   self.write(json_encode(''))
                   #database.execute("INSERT INTO traceinfo (phonenumber,latlong) VALUES (%s,%s)",phonenumber,latlong);
        elif tag=="goodtruck":
             value2=value.strip('"')
             value1=value2.split(",")
             gn=value1[0]
             dn=value1[1]
             if database:
                    rec=database.query("SELECT * FROM goodtraceinfo WHERE goodnumber=%s",gn);
                    if rec:
                        database.execute("UPDATE goodtraceinfo SET trucknumber=%s where goodnumber=%s",dn,gn);
                        
                    else:
                        database.execute("INSERT INTO goodtraceinfo (goodnumber,trucknumber) VALUES (%s,%s)",gn,dn);
             ...
             password=value1[1]
             if database:
                    rec=database.query("SELECT * FROM usernamepw WHERE username=%s",username);
                    if rec:
                        database.execute("UPDATE usernamepw SET password=%s",password);
                    else:
                        pass
        elif tag=='gpsidaddv1':
            ...
            pv=value.strip('"')
            if database:
                   database.execute("INSERT INTO traceinfo (phonenumber,latlong) VALUES (%s,%s)",pv,'');
                   database.execute("INSERT INTO goodtraceinfo (goodnumber,trucknumber) VALUES (%s,%s)",pv,pv);
                   tag='gpsidaddmessage'
        elif tag=='gpsiddelv1':
            pv=value.strip('"')
            #send_mail("13779961531@139.com", tag,value,("smtp.139.com", 25, "13779961531@139.com", "tangjin7624" ,False))
            database.execute("DELETE FROM traceinfo WHERE phonenumber=%s",pv);
        else:
            pass
...
class Admin(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if database:
           rec=database.query("select * from traceinfo  where phonenumber like %s ORDER BY id DESC ",phonenumber);
...
class Delete(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if database:
           rec=database.query("select * from traceinfo  where id=%s",gpsid);
        
           if rec:
              for recsign in rec:
                database.execute("DELETE FROM traceinfo WHERE id=%s",gpsid);
...
class Admingn(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if database:
           rec=database.query("select * from goodtraceinfo  where goodnumber like %s ORDER BY id DESC ",goodnumber);
...
class Username(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if database:
           rec=database.query("select * from usernamepw  where username like %s ORDER BY id DESC ",username);
    ...
    def post(self):
        ...
        if database:
             ...
             if rec:
                ...
                database.execute("INSERT INTO usernamepw (username,password) VALUES (%s,%s)",username,password);
...
class Usernamedelete(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if database:
           rec=database.query("select * from usernamepw  where id=%s",uid);
        
           if rec:
              for recsign in rec:
                database.execute("DELETE FROM usernamepw WHERE id=%s",uid);
                self.write("<center><h3>delete is done</h3></center>")
...
class Deletegn(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if database:
           rec=database.query("select * from goodtraceinfo  where id=%s",gnid);
        
           if rec:
              for recsign in rec:
                database.execute("DELETE FROM goodtraceinfo WHERE id=%s",gnid);
                self.write("<center><h3>delete is done</h3></center>")
...
class WeixinInterface(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if xml.find("MsgType").text=='text':
           if database:
                rec=database.query("SELECT * FROM goodtraceinfo WHERE goodnumber=%s",tracenumber);                                                                                                                                                                                                                                                                                                                                                                     
                if rec:
                    trucknumber=rec[0]['trucknumber']
                    
                    rec2=database.query("SELECT * FROM traceinfo WHERE phonenumber=%s",trucknumber);
                    if rec2:
